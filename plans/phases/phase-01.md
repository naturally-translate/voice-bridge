...
- Monorepo packages: server (`projects/server`), web client (`projects/web-client`), root configs.

## Tasks
- [ ] Create `.tool-versions` with the runtime versions above; install ASDF plugins (nodejs, python, poetry, pnpm) and run `asdf install`.
- [ ] Add TypeScript configs (`tsconfig.base.json`, `projects/server/tsconfig.json`) with strict mode enabled.
...
- [ ] Create interfaces under `projects/server/src/interfaces/`: `IASR.ts`, `ITranslator.ts`, `ITTS.ts`, `IVAD.ts`, `IModelManager.ts`.
- [ ] Implement `ModelRegistry.ts` + `ModelManager.ts` in `projects/server/src/services/model-manager/` for on-demand downloads to `models/`.
...
- `pnpm -C projects/server tsc --noEmit` passes.
