...
**projects/core** (Framework-agnostic domain logic)

- ASR, Translation, TTS, VAD service abstractions
- TranslationPipeline orchestration
- Web Worker architecture for parallelism
- Model management (on-demand downloads)
- Audio processing utilities

**projects/api** (GraphQL API shim)

- Apollo Server with WebSocket subscriptions
- Thin resolvers delegating to Core
- Session management for multi-client support
- PubSub for real-time streaming events

**projects/client-app** (Electron + React control plane)

- Material UI interface
- Apollo Client for GraphQL subscriptions
- Audio input/output via Web Audio API
...
graph TB
    subgraph "User's Machine"
        subgraph "Electron Application Process"
            Main[Main Process<br/>Node.js]
            Renderer[Renderer Process<br/>Chromium + React]

            subgraph "Web Workers (Isolated V8 Contexts)"
                WW1[Translation Worker 1<br/>NLLB Spanish]
                WW2[Translation Worker 2<br/>NLLB Chinese]
                WW3[Translation Worker 3<br/>NLLB Korean]
                WW4[ASR Worker<br/>Parakeet TDT]
                WW5[TTS Worker Pool<br/>XTTS Clients 3x]
            end

            Worklet[Audio Worklet<br/>VAD Processing]
        end

        subgraph "GraphQL API Process"
            Apollo[Apollo Server<br/>Express + WebSocket]
            Core[Core Pipeline<br/>Orchestration]
        end

        subgraph "Python Process (FastAPI)"
            XTTS[XTTS-v2 Server<br/>Port 8000<br/>HTTP API]
        end

        subgraph "File System"
            Models[models/ Directory<br/>Cached ML Models<br/>~8GB Storage]
            Config[Config Files<br/>settings.json]
        end
    end

    subgraph "External Services"
        HF[Hugging Face Hub<br/>Model Downloads<br/>HTTPS]
    end
...
graph TB
    subgraph "Client Application UI"
        App[App.tsx<br/>Apollo Provider + Theme]

        App --> Layout[MainLayout.tsx<br/>MUI Container]

        Layout --> Header[Header.tsx<br/>App Title + Settings]
        Layout --> Main[MainContent.tsx<br/>Grid Layout]

        Main --> Control[ControlPanel.tsx]
        Main --> Display[TranslationDisplay.tsx]

        subgraph "Control Panel Components"
            Control --> StartBtn[StartButton.tsx<br/>Start/Stop Pipeline]
            Control --> LangSelect[LanguageSelector.tsx<br/>Enable/Disable Languages]
            Control --> AudioCtrl[AudioControls.tsx<br/>Volume + Input Device]
            Control --> ModelStatus[ModelStatusPanel.tsx<br/>Download Progress]
        end

        subgraph "Translation Display Components"
            ...
            TargetPanels --> SpanishPanel[SpanishPanel.tsx<br/>Text + Audio Indicator]
            TargetPanels --> ChinesePanel[ChinesePanel.tsx<br/>Text + Audio Indicator]
            TargetPanels --> KoreanPanel[KoreanPanel.tsx<br/>Text + Audio Indicator]
        end

        Layout --> Footer[Footer.tsx<br/>Status Bar + Stats]

        Footer --> Status[StatusIndicator.tsx<br/>Pipeline State]
        Footer --> Latency[LatencyMonitor.tsx<br/>Performance Metrics]
        Footer --> Memory[MemoryUsage.tsx<br/>RAM Monitor]
    end

    subgraph "GraphQL Subscriptions"
        Sub1[streamTranscription]
        Sub2[streamTranslation<br/>lang: es]
        Sub3[streamTranslation<br/>lang: zh]
        Sub4[streamTranslation<br/>lang: ko]
        Sub5[streamAudio<br/>3x languages]
        Sub6[pipelineStatus]
    end

    SourcePanel -.->|Subscribe| Sub1
    SpanishPanel -.->|Subscribe| Sub2
    ChinesePanel -.->|Subscribe| Sub3
    KoreanPanel -.->|Subscribe| Sub4
    Display -.->|Subscribe| Sub5
    Status -.->|Subscribe| Sub6
...
1. **XTTS-v2 as Python Microservice**: No mature TypeScript implementation exists; Python backend with HTTP API required for prosody extraction
2. **NLLB-200 Distilled (600M)**: Fits 3 instances in memory (~3.6GB total)
3. **Web Workers for Parallelism**: 3 translation workers + 3 TTS workers for simultaneous language processing
4. **GraphQL Subscriptions**: Standard protocol for multi-client real-time streaming
5. **On-Demand Model Downloads**: Transformers.js auto-download, cached in `models/` directory
